...
//----------
// TEMP DEBUG HELPERS — layout testing only
window.debugFillCellDots = function (day, time, count = 10) {
  const cell = table?.querySelector(
    `td[data-day="${String(day)}"][data-time="${String(time)}"]`
  );

  if (!cell) {
    console.warn("debugFillCellDots: cell not found", { day, time });
    return;
  }

  // Make sure gold styling does not hide the dots while testing
  cell.classList.remove("gold-cell");

  // Remove current visible dot container only (DB not touched)
  ...
  for (let i = 1; i <= count; i++) {
    ...
    dot.className = "dot";
    dot.dataset.userId = `debug-user-${i}`;
    dot.dataset.name = `tester${i}`;
    dot.dataset.debug = "1";
    dot.title = `tester${i}`;
    dot.style.background = debugColors[(i - 1) % debugColors.length];
    dc.appendChild(dot);
  }

  if (typeof refreshDotLayout === "function") {
    refreshDotLayout(cell);
  }

  console.log(`debugFillCellDots: rendered ${count} dots in`, { day, time, cell });
};

window.debugClearCellDots = async function (day, time, reloadFromDb = true) {
  const cell = table?.querySelector(
    `td[data-day="${String(day)}"][data-time="${String(time)}"]`
  );

  if (!cell) {
    console.warn("debugClearCellDots: cell not found", { day, time });
    return;
  }
  ...
  if (reloadFromDb && typeof rebuildDotsForCell === "function") {
    await rebuildDotsForCell(cell);
    if (typeof applyGoldStateForCell === "function") {
      await applyGoldStateForCell(cell, cell.dataset.day);
    }
  }

  console.log("debugClearCellDots: cleared", { day, time, reloadFromDb });
};
